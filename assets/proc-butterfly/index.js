...
var replace_butterfly = null;

window.onhashchange = function(e) {
  rng = new Math.seedrandom(location.hash.slice(1));
  if (replace_butterfly != null) {
    replace_butterfly();
  }
}

window.onload = function() {
  document.getElementById('another_one').onclick = function() {
  ...
  var prev_wing_rotation = 0;
  var wing_flat_speed_coef = 0;
  replace_butterfly = function() {
    for (var i = 0; i < butterfly.length; i++) {
      butterfly[i].geometry.dispose();
      butterfly[i].material.dispose();
      scene.remove(butterfly[i]);
    }

    prev_wing_rotation = 0;
    ...
    wing_flat_speed_coef = randrange(2.5, 5);
  }

  replace_butterfly();

  function render(time) {
    camera.position.x = Math.cos(time / 1000 * 2) * 4;
    camera.position.y = Math.sin(time / 1000 * 2) * 4;
    camera.lookAt(new THREE.Vector3(0, 0, 0));

    var wing_rotation = Math.sin(time / 1000 * wing_flat_speed_coef) * (Math.PI * .4);
    for (var i = 0; i < butterfly.length; i++) {
      if (butterfly[i].wing_rotation_type !== undefined) {
        if (butterfly[i].wing_rotation_type == 'right') {
          butterfly[i].rotation.y += wing_rotation - prev_wing_rotation;
        } else {
          butterfly[i].rotation.y -= wing_rotation - prev_wing_rotation;
        }
      }
    ...
    prev_wing_rotation = wing_rotation;
